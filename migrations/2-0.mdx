...
description: "Prepare an existing @bilyai/js integration for the 2.0 initialization, readiness, payload, and event contract."
---

Version 2.0.0 modernizes the SDK while retaining compatibility overloads for older initialization code. Confirm that `2.0.0` is available from your package registry before updating a production dependency.

```bash
npm view @bilyai/js version
```

When the registry reports `2.0.0` or newer, install the release:

```bash
npm install @bilyai/js@^2.0.0
```
...
In 2.0, prefer the exact URL copied from **Bily > Settings > Tracking**:
