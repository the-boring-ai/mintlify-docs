...
`useRouter().afterEach()` runs after Nuxt's router commits a navigation. It also observes query-only changes when Nuxt reuses the current page component. Comparing the complete URL prevents repeated lifecycle notifications from creating duplicates.
