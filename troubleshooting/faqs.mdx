...
<AccordionGroup>
  ...
  <Accordion title="Why can’t I see an event even though ready() resolved?">
    ...
    Confirm that the successful product path reached `track()` once. Check that the event name is non-empty, the page uses the intended store URL, consent allows the call, and the observed payload matches the event plan.
  </Accordion>
