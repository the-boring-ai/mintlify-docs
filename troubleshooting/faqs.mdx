...
<AccordionGroup>
  <Accordion title="Should I use the raw script or the npm package?">
    Use the exact raw script for plain HTML or a site-managed script field. Use `@bilyai/js` for applications with an npm build.

    Install only one method on each website surface.
  </Accordion>
  <Accordion title="Which frameworks does the Bily SDK support?">
    The SDK supports browser applications built with Vanilla JavaScript, React Router, Next.js, Vue, Nuxt, Svelte, SvelteKit, Angular, Astro, React Router framework mode, and Remix.

    Each framework uses the same SDK contract and its own committed-navigation lifecycle. Choose the exact integration in [Framework guides](/sdk/overview).
  </Accordion>
  ...
  <Accordion title="Does the tracking URL need a private token?">
    No. The tracking URL is public installation data because every visitor's browser must load it.

    Copy it exactly, but never add a private token, API key, session value, or authorization header to browser configuration.
  </Accordion>
  <Accordion title="When does the tracking URL change to my subdomain?">
    Keep the provided URL while you validate the installation. After Bily validates your customer subdomain, copy the new complete URL from Bily and replace the old value as one deployment change.

    Do not change only the hostname or run both URLs together. Follow [Move to first-party tracking](/guides/first-party-tracking).
  </Accordion>
  <Accordion title="Do I need to rebuild after changing the tracking URL?">
    Yes when a public environment value is included in browser assets. This includes common `NEXT_PUBLIC_`, `VITE_`, `$env/static/public`, Astro `PUBLIC_`, and compiled Angular settings.

    A runtime-served value may need only a restart or redeploy. Open a new document with a full reload after deployment; a hot update cannot replace the active Bily URL.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Are llms.txt and llms-full.txt MCP endpoints?">
    ...
    Use `https://api.bily.ai/mcp` as the MCP endpoint.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Why do I see duplicate page views?">
    Common causes include adding an unmanaged script tag alongside the SDK, sending a manual initial `PageView`, registering several router listeners, or failing to deduplicate the current URL.
