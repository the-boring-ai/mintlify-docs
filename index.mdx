...
description: "Install governed web capabilities, send browser events, and build trusted server workflows with Bily."
---

Bily is an app store for the internet: one trusted place to install, govern, and evolve reusable web capabilities. Connect a site once, use the SDK for browser events, and use the API for authenticated server workflows.

Installing a capability should feel like installing an app: review its permissions, configure it, enable it, and later update or revoke it from one place.
...
The API is the server contract. It authenticates your backend, applies organization and store boundaries, and exposes the Bily capabilities available to your account.

Both surfaces keep the customer-facing integration stable while Bily capabilities improve over time.
